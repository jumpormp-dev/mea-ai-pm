...
# --- 6. SIDEBAR: SYNC & SURVEY & UPLOAD ---
with st.sidebar:
    st.header("⚙️ การจัดการข้อมูล")
    
    # 1. Sync ข้อมูลเว็บ (172.16.111.184)
    if st.button("📡 Sync Smart Meter"):
        if not st.session_state.ktd_assets.empty:
            ...
            st.success(f"ซิงค์ข้อมูล {size} เครื่องสำเร็จ")
        else:
            st.warning("กรุณาอัปโหลดไฟล์ ฟขต. ก่อน")

    # 2. อัปโหลดไฟล์ ฟขต Feeder.xlsx (รองรับข้อมูลจริงทั้งหมด)
    ...
    if uploaded_xlsx:
        ...
        if 'Feeder' in df_xlsx.columns:
            ...
            if st.button("🚀 โหลดข้อมูลจริงเข้าระบบ"):
                new_data = []
                for i, fdr in enumerate(unique_feeders):
                    new_data.append({
                        ...
                        'Plan_Month': '-', 'Survey_Img': None
                    })
                st.session_state.ktd_assets = pd.DataFrame(new_data)
                st.success(f"โหลดข้อมูลจริง {len(unique_feeders)} รายการสำเร็จ")

    st.divider()
    
    # 3. บันทึกข้อมูลสำรวจ + อัปโหลดรูป
    if not st.session_state.ktd_assets.empty:
        ...
        # ช่องอัปโหลดภาพหน้างาน
        img_file = st.file_uploader("อัปโหลดภาพหน้างาน (JPG/PNG)", type=["jpg", "png", "jpeg"], key=f"img_{target_id}")

        if st.button("💾 บันทึกข้อมูลและวิเคราะห์"):
    ...
    if st.button("🚀 วิเคราะห์ภาพรวมทั้งหมด"):
...
# --- 7. MAIN CONTENT ---
tab1, tab2, tab3 = st.tabs(["📊 Executive Summary", "🔍 Asset Diagnostics", "📅 Maintenance Plan"])

with tab1:
    if st.session_state.ktd_assets.empty: st.info("👈 กรุณาอัปโหลดไฟล์ ฟขต. เพื่อเริ่มต้น")
    else:
        df = st.session_state.ktd_assets
...
with tab2:
    if not st.session_state.ktd_assets.empty:
        ...
        with cl:
            ...
            if res['Survey_Img']: st.image(res['Survey_Img'], caption=f"ภาพหน้างาน {sel_id}", use_container_width=True)
        with cr:
